...
// 1. Sorteia o clube.
// 2. Escolhe o jogador.
// 3. Depois clica na posição.
// 4. O jogador só pode ser colocado na posição dele.
// 5. Depois que um jogador é escolhido, aquele clube não pode
//    mais ser sorteado.
// ============================================================
            ...
            ${
              row
                .map(position => {
                  ...
                  // Posição correta do jogador escolhido
                  if(
                    state.pendingPlayer &&
...
function drawClub(){
  ...
  // Não deixa sortear enquanto há jogador aguardando
  // ser colocado.
  ...
  const available =
    CLUBS.filter(
      club =>
        !state.usedClubs.includes(club)
    );

  if(!available.length){

    alert(
      "Todos os clubes disponíveis já foram utilizados."
    );
...
function choosePlayer(index){
  ...
  if(state.pendingPlayer){
    ...
    alert(
      "Você já escolheu um jogador. Agora coloque ele no campo."
    );
  ...
  // Guarda o jogador.
  // Ele ainda NÃO foi colocado no time.
  state.pendingPlayer = {
  ...
  $("instruction").textContent =
    `${player[0]} escolhido. Agora clique na posição ${player[1]}.`;
...
function confirmTeam(){
  ...
  if(
    ...
    alert(
      "Você precisa colocar os 11 jogadores."
    );
  ...
  state.peer.on(
    ...
    ()=>{
      ...
      $("roomStatus").textContent =
        `🟢 Sala criada: ${roomId}. Envie este código para seu amigo.`;
